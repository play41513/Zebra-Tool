...
bool cFileControl::FindINIFile()
{
	...
	else
	{
		...
		INI_Desc->JigNumber = INI_Desc->ININame.SubString(INI_Desc->ININame.Pos("_")+1,INI_Desc->ININame.Length()-INI_Desc->ININame.Pos("_"));
		INI_Desc->ININame = INI_Desc->ININame.SubString(1,INI_Desc->ININame.Pos("_")-1);
		ReadINIFile();
		delete FileList;
		return true;
	}
...
void cFileControl::ReadINIFile()
{
	...
	for(int i = 0;i < (int)INI_Desc->dwPSUSettingCount ; i++)
	{
		...
		INI_Desc->PITEM_DESCRIPOR[i].PSU_OutputAfterDelay
        	= ini->ReadInteger("Parameter","PSU_OutputAfterDelay",1);
