...
//---------------------------------------------------------------------------
void TFrmMain::Voltage_Test(AnsiString CMD,TPanel *pl_main,TPanel *pl_read,float MinVoltage,float MaxVoltage)
{
	...
	while(bl)
	{
		switch(step)
		{
			...
			case TEST_VOLTAGE_PASS:
				...
				if(atof(m_ADValue.c_str())<0) m_ADValue = "0.000";
...
bool TFrmMain::ReadInISet()
{
	if(FileExists(FILE_DUT_SET_INI))
	{
		...
		for(int i = 0 ;i < 15;i++)
		{
			...
			edtADSetMin->Text = sList->Strings[2];
			edtADSetMax->Text = sList->Strings[3];
		}
		AnsiString PSUSET = Findfilemsg(FILE_DUT_SET_INI,DEFAULT_DEVICE,16);
		edtPSUVol->Text = PSUSET.SubString(11,PSUSET.Pos("~")-11);
		edtPSUCur->Text = PSUSET.SubString(PSUSET.Pos("~")+1,PSUSET.Length());
		AnsiString DUTCURRENT = Findfilemsg(FILE_DUT_SET_INI,DEFAULT_DEVICE,17);
		edt_min->Text = DUTCURRENT.SubString(15,DUTCURRENT.Pos("~")-15);
		edt_max->Text = DUTCURRENT.SubString(DUTCURRENT.Pos("~")+1,DUTCURRENT.Length());
			...
			while (!imsgfile.eof()) {
				...
				if (item_count==1) {
					...
					if(dut_info_count <15)
					{
						...
						totalmsg +=  "\n" + AnsiString(msg.c_str()).SubString(1,7) + Auto +edtADSetMin->Text+" "+edtADSetMax->Text;
						dut_info_count++;
					}
...
void __fastcall TFrmMain::plAD0Click(TObject *Sender)
{
	...
	else if(numBtn <=14)
	{
		...
		float minvoltage = ((TEdit *)FindComponent("edtADSetMin" + IntToStr(numBtn)))->Text.ToDouble();
		float maxvoltage = ((TEdit *)FindComponent("edtADSetMax" + IntToStr(numBtn)))->Text.ToDouble();
