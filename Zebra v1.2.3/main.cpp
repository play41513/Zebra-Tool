	...
	if(FileExists(FILE_DUT_SET_INI))
	{
		try
		{
			...
			for(int i = 0 ;i < 15;i++)
			{
				...
				plPN->Text        = sList->Strings[4];
				edtKValue->Text   = sList->Strings[5];
			}
			...
			while (!imsgfile.eof()) {
				...
				if (item_count==1) {
					...
					if(dut_info_count <15)
					{
						...
						totalmsg +=  "\n" + AnsiString(msg.c_str()).SubString(1,7) + Auto +edtADSetMin->Text+" "+edtADSetMax->Text+" "+plPN->Text+" "+edtKValue->Text;
						dut_info_count++;
					}
...
bool TFrmMain::FindIniFile()
{
	...
	else
	{
		...
		if(FILE_DUT_SET_INI.Length()>12)
			labIni->Caption = FILE_DUT_SET_INI.SubString(8,FILE_DUT_SET_INI.Length()-11);
		delete FileList;
		return true;
	}
}
