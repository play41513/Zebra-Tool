	...
	while(bl)
	{
		switch(step)
		{
			...
			case READ_AD_VALUE:
				...
				//  2020/3/16 更改為無條件捨去
					m_ADValue = AnsiString(floor(m_ADValue.ToDouble()*100));
					m_ADValue = AnsiString(m_ADValue.ToDouble()/100);
				// K Value
			...
			case TEST_VOLTAGE_PASS:
				...
				if(MinVoltage == 0) pl_main->Font->Color = clGreen;
...
bool TFrmMain::ReadInISet()
{
	if(FileExists(FILE_DUT_SET_INI))
	{
		try
		{
			...
			sTemp = Findfilemsg(FILE_DUT_SET_INI,"[OVP-DUT-CURRENT]",0);
			if(sTemp!="")
			{
				g_OVP_VOL_MIN = sTemp.SubString(19,sTemp.Pos("~")-19).ToDouble();
				g_OVP_VOL_MAX = sTemp.SubString(sTemp.Pos("~")+1,sTemp.Length()).ToDouble();
			}
			sTemp = Findfilemsg(FILE_DUT_SET_INI,"[OVP-LED]",0);
			if(sTemp!="")
			{
				bLED = sTemp.Pos("true")?true:false;
			}
			else bLED = false;

			sTemp = Findfilemsg(FILE_DUT_SET_INI,"[OCP]",0);
			if(sTemp!="")
			{
				bOCP = sTemp.Pos("true")?true:false;
			}
			else bOCP = true;
...
bool TFrmMain::SetInIVal(){
	...
	while(1)
	{
		...
		else if(dut_info_count ==20)
		{
			AnsiString ckb = ckb_OVP->Caption == "V" ? "1 ":"0 ";
			totalmsg +=  "\n[OVP-DUT-CURRENT] "
				+AnsiString(g_OVP_VOL_MIN)+"~"+ AnsiString(g_OVP_VOL_MAX);
			dut_info_count++;
		}
		else if(dut_info_count ==21)
		{
			AnsiString led = bLED ? "true":"false";
			totalmsg +=  "\n[OVP-LED] "+led;
			dut_info_count++;
		}
		else if(dut_info_count ==22)
		{
			AnsiString ocp = bOCP ? "true":"false";
			totalmsg +=  "\n[OCP] "+ocp;
			break;
		}
	}
		...
		if(atof(AnsiString(pl_read_voltage_ad_15->Caption).c_str())< 1&&!bAutoStart&&Timer1->Enabled&&pl_read_voltage_ad_15->Caption!="")
		{
			...
			if(ckb_OVP->Caption=="V"&&!bLED)
			{
				if(CL_PSU_CONTROL->open_psu_com())
				{
					...
					switch(CL_PSU_CONTROL->PSU_CMD_Test(true))
					{
						case TEST_PASS:
							CL_PSU_CONTROL->PSU_Stop();
							break;
						case TEST_FAIL:
							ERROR_MSG = MSG_OVP_CURRENT_FAIL;
							pl_sys_power->Color = clRed;
							bError = true;
							CL_PSU_CONTROL->PSU_Stop();
							break;
			...
			{
				//PSU Output
				switch(PSU_Voltage_Test())
				{
					case TEST_PASS:
						pl_sys_power->Color = clLime;
						btnStart->Click();
						break;
					case TEST_FAIL:
						pl_sys_power->Color = clRed;
			...
			//SSC3 OVP Test
			if(ckb_OVP->Caption=="V"&&!bCurFail&&bLED)
			{
				//que_cmd.c.clear();
				char buff[200];
				sprintf(buff,"CHAN1:VOLTage %.4f\n",edtOVPVol->Text.ToDouble());      //設定電壓
				...
				plOVP_ch->Caption = MSG_CHECK_LED_COLOR_CHINESE;
				plOVP_eng->Caption = MSG_CHECK_LED_COLOR_ENGLISH;
...
int TFrmMain::PSU_Voltage_Test()
{
	if(CL_PSU_CONTROL->open_psu_com())
	{
		...
		if(bOCP)
        	...
        	que_cmd.push("CHAN1:PROTection:CURRent 0\n");
		que_cmd.push(":OUTPut:STATe 1\n");                                    //開始輸出
		...
		return CL_PSU_CONTROL->PSU_CMD_Test(false);
	}
	else
	{
...
void __fastcall TFrmMain::edtADSetMin0Exit(TObject *Sender)
{
	...
	else if(edt->Text.Length() == 0 || edt->Font->Color == clRed||edt->Text.ToDouble() <0.01){
		edt->Text = "0.00";
		edt->Font->Color = clBlue;
	}
	else
...
void __fastcall TFrmMain::btnSetClick(TObject *Sender)
{
	if(btnSet->Caption == "Enter")
	{
		...
		{
			bAutoStart = true;
			pl_read_voltage_ad_15->Caption = "Setting...";
			btnSet->Caption		 = "END";
...
bool TFrmMain::writeLOG(AnsiString Msg,bool bAction)
{
	...
	else
	{
		...
		AnsiString TPVoltage = "";
		for(int i = 0 ; i <= 14 ; i++)
		{
			if(((TPanel *)FrmMain->FindComponent("pl_Auto"+AnsiString(i)))->Caption == "V")
			{
				TPVoltage += " ["
				+((TPanel *)FrmMain->FindComponent("pl_pin"+AnsiString(i)))->Caption
				+ "]"
				+((TPanel *)FrmMain->FindComponent("pl_read_voltage_ad_"+AnsiString(i)))->Caption;
			}
		}
		if(Msg.Pos("PASS"))
		{
			SS = strTemp+" [PASS]"+AnsiString(FormatDateTime("mm-dd-yyyy hh:mm:ss", Now()))
				+" [NAME]"+labProduct->Caption+" [PSU_DATA]"+pl_sys_power->Caption.Delete(1,15)+TPVoltage;
		}
		else
		{
			SS = strTemp+" [FAIL]"+AnsiString(FormatDateTime("mm-dd-yyyy hh:mm:ss", Now()))
				+" [NAME]"+labProduct->Caption+" [PSU_DATA]"+pl_sys_power->Caption.Delete(1,15)+TPVoltage
				+" [ERROR]"+pl_Msg->Caption;
		}
		SS+= "\r\n";
		bPASS[0] = SaveLogLine(RDLOGFilePath,SS);
		...
		if(LOGDiskPath.Pos("log") && ImgDisk->Visible)
		{
			...
			if(!bPASS[2] || !bPASS[2])
			{
				MessageBoxA(FrmMain->Handle,"LOG隨身碟異常","WARNING", MB_ICONEXCLAMATION);
...
bool TFrmMain::CheckDiskName()
{
	...
	for(int i =3;i<=25;i++)
	{
		if((DiskInfo&(0x01<<i))!=0)
		{
			...
			{
				LOGDiskPath = DiskName+"\\ASMP\\log\\Zebra";
	...
	if(DirectoryExists(LOGDiskPath.c_str()))
	{
		...
		else//檔案已存在於USBDisk
		{
			if(!FileExists(LOGFilePath))
			{
				...
				CopyFileA(RDLOGDiskPath.c_str(),RDLOGFilePath.c_str(),true);
			}
		}
	}
	else
